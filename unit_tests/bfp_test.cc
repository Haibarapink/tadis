...
class BufferPoolTester {
public:
  void reopen()
  {
    BufferPool bfp{"reopen.db"};
    size_t page_count = 1024;
    std::vector<PageId> pids;
    for (auto i = 0; i < page_count; ++i) {
      PageId pid;
      assert(bfp.new_page(pid));
      assert(pid == i + 1);
      bfp.unpin(pid, true);
    }
    assert(bfp.head_.page_num_ == page_count + 1);
    assert(bfp.head_.phy_num_ == page_count + 1);
    bfp.delete_page(510);
    assert(bfp.head_.bitmap().get(510) == false);
    bfp.close();

    BufferPool bfp2{"reopen.db"};
    assert(bfp2.head_.page_num_ == page_count);
    assert(bfp2.head_.phy_num_ == page_count + 1);
    assert(bfp2.head_.bitmap().get(510) == false);
    bfp2.close();
