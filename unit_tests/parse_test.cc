...
void basic_test()
{
  ...
  BOOST_TEST(std::get<SelectAst>(p.query()).selist_.front().attribute_ == "*");
}
...
void selist_test()
{
  ...
  Parser<std::string> p(s);
  BOOST_TEST(p.parse() == RC::SUCCESS);
  BOOST_TEST(std::get<SelectAst>(p.query()).selist_[0].attribute_ == "age");
  BOOST_TEST(std::get<SelectAst>(p.query()).selist_[1].attribute_ == "name");
  BOOST_TEST(std::get<SelectAst>(p.query()).selist_[2].attribute_ == "name");
  BOOST_TEST(std::get<SelectAst>(p.query()).selist_[2].table_ == "school");
}
...
void from_test()
{
  ...
  Parser<std::string> p(s);
  BOOST_TEST(p.parse() == RC::SUCCESS);
  auto select = std::get<SelectAst>(p.query());
...
void delete_test1()
{
  ...
  Parser<std::string> p{s};
  BOOST_TEST(p.parse() == RC::SUCCESS);
  auto d = std::get<DeleteAst>(p.query());
...
void insert_test1()
{
  ...
  auto insert = std::get<InsertAst>(parser.query());
...
void create_test1()
{
  ...
  Parser<std::string> parser{s};
  BOOST_TEST(parser.parse() == RC::SUCCESS);
  auto c = std::get<CreateTableAst>(parser.query());
