...
private:
  struct state {
    ...
    inline state(char *data, size_t size) : data_(data), size_(size), offset_(0)
    {}

    inline state() : data_(nullptr), size_(0), offset_(0)
    {}
    ...
    inline void set(char *data, size_t len)
    {
      reset();
      data_ = data;
      size_ = len;
...
public:
  inline Parser(const std::string &data) : state_(const_cast<char *>(data.data()), data.size())
  {}

  inline Parser(char *data, size_t size) : state_(const_cast<char *>(data), size)
  {}
  ...
  {
    Reset();
    state_.set(const_cast<char *>(data.data()), data.size());
  }
};
...
inline bool Parser::parse_true(Value &v)
{
  ...
  if (offset + 3 >= state_.size_ || d[offset + 1] != 'r' || d[offset + 2] != 'u' || d[offset + 3] != 'e')
    return false;
...
inline bool Parser::parse_false(Value &v)
{
  ...
  if (offset + 4 >= state_.size_ || d[offset + 1] != 'a' || d[offset + 2] != 'l' || d[offset + 3] != 's' ||
      d[offset + 4] != 'e')
    return false;
...
inline bool Parser::parse_null(Value &v)
{
  ...
  if (offset + 3 >= state_.size_ || d[offset + 1] != 'u' || d[offset + 2] != 'l' || d[offset + 3] != 'l')
    return false;
