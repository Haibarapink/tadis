...
#define Serialize_Start               \
  inline pson::Value Serialize()      \
  {                                   \
    pson::Value v{pson::JSON_OBJECT}; \
    pson::Object &obj = v.as<pson::Object>();

#define Serialize_Insert(a) \
  pson::enable_if<decltype(a), IS_JSON_TYPE(decltype(a))>().ObjectInsert(obj, std::move(std::string(VALUE_NAME(a))), a);
...
#define DeSerialize_Start                          \
  inline void DeSerializeFromValue(pson::Value &v) \
  {                                                \
    auto &&obj = v.as<pson::Object>();

#define DeSerialize_Fetch(a) \
  pson::enable_if<decltype(a), IS_JSON_TYPE(decltype(a))>().ObjectFetch(obj, std::string(VALUE_NAME(a)), a);
